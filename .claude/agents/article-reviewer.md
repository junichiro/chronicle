...
- 有効なリスト書式
- **太字の前後スペース（重要）** : 日本語テキスト内の `**text**` の前後に半角スペースが必要。スペースがない場合は追加する。既にスペースがある場合は削除しない（パーサーの互換性のため必須）
...
- **ブロック数式の使用確認**: 複雑な数式は `$$...$$` で表示されているか
- **インライン数式の適切さ**: `$...$` は変数名や短い記号のみに限定されているか
- MathJax 構文の正しさ（括弧の対応、コマンドのスペル）
- 数式前後に適切な空行があるか

**チェック例**:
- NG: 文中に `$x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$` のような複雑な数式
- OK: ブロックで表示、インラインは `$x$` や `$n$` のみ
